...
struct EmulationView: View {
    ...
    @State private var currentActivity: Activity<EmulationAttributes>?
    ...
    private func startEmulation() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { _ in
            if timeRemaining > 0 {
                timeRemaining -= 1
                updateLiveActivity(status: "Emulating on Bridge")
            } else {
                stopEmulation()
            }
        }
    ...
    private func startDirectEmulation() {
        ...
        startLiveActivity(status: "Direct Emulation (Jailbreak)")
        
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { _ in
            if timeRemaining > 0 {
                timeRemaining -= 1
                updateLiveActivity(status: "Direct Emulation (Jailbreak)")
            } else {
                stopDirectEmulation()
            }
        }
    ...
    private func startLiveActivity(status: String) {
        guard ActivityAuthorizationInfo().areActivitiesEnabled else { return }
        
        let attributes = EmulationAttributes(cardName: card.name)
        let state = EmulationAttributes.ContentState(timeRemaining: timeRemaining, status: status)
    ...
    private func endLiveActivity() {
        guard let activity = currentActivity else { return }
        let state = EmulationAttributes.ContentState(timeRemaining: 0, status: "Stopped")
